""" Defines a U-Net model for MCDropout. """
import time
...
class UNetDropout:
    def __init__(self, load_path=None, lr=None):
        ...
        self._train_phase_predict = K.function(
    ...
    def dropout_predict(self, x, num_samples=8):
        batch_size = x.shape[0]
        out_shape = self.model.layers[-1].output.shape[1:]
        collected = np.zeros((num_samples,) + (batch_size,) + tuple(out_shape))
        for i in range(num_samples):
            collected[i, ...] = self._train_phase_predict((x, 1))[0]

        mean = collected.mean(axis=0)
        variance = collected.var(axis=0, ddof=1)
