...
class IntervalConv1d(torch.nn.Conv1d):
	...
	def forward_min_max(self, v_min, v_max):
		if torch.all(v_min == v_max):
			previous_value_min = torch.nn.functional.conv1d(torch.clamp(v_min,0,None),self.min_weight,					**self.key_word_args)	\
								+torch.nn.functional.conv1d(torch.clamp(v_min,None,0),self.max_weight,					**self.key_word_args)	\
								+self.min_bias[None,:,None]

			previous_value_max = torch.nn.functional.conv1d(torch.clamp(v_min,0,None),self.max_weight,					**self.key_word_args)	\
								+torch.nn.functional.conv1d(torch.clamp(v_min,None,0),self.min_weight,					**self.key_word_args)	\
								+self.min_bias[None,:,None]
