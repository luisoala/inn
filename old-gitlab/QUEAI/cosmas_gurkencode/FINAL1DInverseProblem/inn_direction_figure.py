...
def direction_test(pred, target, lower, upper, t):
	...
	return tid_same_rate, tid_opposite_rate, np.average(weights) #np.sum(weights)

all_data = np.load(data_path+'interval_test_dict.npz.npy', allow_pickle=True)
all_data = all_data[np.newaxis]
all_data = all_data[0]
num_data_points = 200.
#num_data_points = all_data.size()[0]
...
ax1.plot(interval_ratios, direction_accuracy, color ='red', ls='--', marker='x', markersize=7, alpha=0.8)
...
ax1.set_xlabel(r'$\mathrm{Interval\ Ratios}$', fontsize=BIGGER_SIZE) #r'$\mathrm{Iterations}$',fontsize=15
