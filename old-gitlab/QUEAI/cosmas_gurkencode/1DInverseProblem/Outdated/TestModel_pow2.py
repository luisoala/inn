...
def run_test(model, data_index):
	data_input, data_output = load_batch_pow_2([data_index])
...
for i in range(num_test_samples):
	...
	for ind in range(len(snapshot_epochs)):
		...
		for beta in betas:
			model.to('cpu')
			model.load_state_dict(torch.load(dir_path+'/ModelSnapshots/pow2_epoch{}_b{}.pt'.format(snapshot_epoch+1,str(beta).split('.')[-1])))
			model.to(device)
