...
from keras_interval_networks.layers import Conv2D, MaxPooling2D, Dropout
...
class UNetInterval():
    ...
    def __init__(self, load_path=None, load_path_interval=None):
        ...
        self.set_interval_trainable(split=-12)
    ...
    def get_layer_creator(self):
        ...
        conv1a = Conv2D(64, 3, activation='relu', padding='same',
                        kernel_initializer='he_normal')
        conv1b = Conv2D(64, 3, activation='relu', padding='same',
                        kernel_initializer='he_normal')
        pool1 = MaxPooling2D(pool_size=(2, 2))
        drop1 = Dropout(0.7)
        # second level convolutional group
        conv2a = Conv2D(128, 3, activation='relu', padding='same',
                        kernel_initializer='he_normal')
        conv2b = Conv2D(128, 3, activation='relu', padding='same',
                        kernel_initializer='he_normal')
        pool2 = MaxPooling2D(pool_size=(2, 2))
        drop2 = Dropout(0.7)
        # third level convolutional group
        conv3a = Conv2D(256, 3, activation='relu', padding='same',
                        kernel_initializer='he_normal')
        conv3b = Conv2D(256, 3, activation='relu', padding='same',
                        kernel_initializer='he_normal')
        pool3 = MaxPooling2D(pool_size=(2, 2))
        drop3 = Dropout(0.7)
        # fourth level convolutional group
        conv4a = Conv2D(512, 3, activation='relu', padding='same',
                       kernel_initializer='he_normal')
        conv4b = Conv2D(512, 3, activation='relu', padding='same',
                       kernel_initializer='he_normal')
        pool4 = MaxPooling2D(pool_size=(2, 2))
        drop4 = Dropout(0.7)
        # fifth level convolutional group
        conv5a = Conv2D(1024, 3, activation='relu', padding='same',
                        kernel_initializer='he_normal')
        conv5b = Conv2D(1024, 3, activation='relu', padding='same',
                        kernel_initializer='he_normal')
        # fourth level upsampling
        up4 = UpSampling2D(size=(2, 2))
        upconv4 = Conv2D(512, 2, activation='relu', padding='same',
                         kernel_initializer='he_normal')
        # fourth level merge
        merge4 = Concatenate(axis=3)
        drop42 = Dropout(0.7)
        # fourth level second convolutional group
        conv42a = Conv2D(512, 3, activation='relu', padding='same',
                         kernel_initializer='he_normal')
        conv42b = Conv2D(512, 3, activation='relu', padding='same',
                         kernel_initializer='he_normal')
        # third level upsampling
        up3 = UpSampling2D(size=(2, 2))
        upconv3 = Conv2D(256, 2, activation='relu', padding='same',
                         kernel_initializer='he_normal')
        # third level merge
        merge3 = Concatenate(axis=3)
        drop32 = Dropout(0.7)
        # third level second convolutional group
        conv32a = Conv2D(256, 3, activation='relu', padding='same',
                         kernel_initializer='he_normal')
        conv32b = Conv2D(256, 3, activation='relu', padding='same',
                         kernel_initializer='he_normal')
        # second level upsampling
        up2 = UpSampling2D(size=(2, 2))
        upconv2 = Conv2D(128, 2, activation='relu', padding='same',
                         kernel_initializer='he_normal')
        # second level merge
        merge2 = Concatenate(axis=3)
        drop22 = Dropout(0.7)
        # second level second convolutional group
        conv22a = Conv2D(128, 3, activation='relu', padding='same',
                         kernel_initializer='he_normal')
        conv22b = Conv2D(128, 3, activation='relu', padding='same',
                         kernel_initializer='he_normal')
        # first level upsampling
        up1 = UpSampling2D(size=(2, 2))
        upconv1 = Conv2D(64, 2, activation='relu', padding='same',
                         kernel_initializer='he_normal')
        # first level merge
        merge1 = Concatenate(axis=3)
        drop12 = Dropout(0.7)
        # first level second convolutional group
        conv12a = Conv2D(64, 3, activation='relu', padding='same',
                         kernel_initializer='he_normal')
        conv12b = Conv2D(64, 3, activation='relu', padding='same',
                         kernel_initializer='he_normal')
        # output layer
        convout = Conv2D(1, 1, activation=None, padding='same',
                         kernel_initializer='he_normal')
    ...
    def set_interval_trainable(self, split=-12):
    ...
    def train(self, epochs, batch_size, train_generator, val_generator,
              verbose=0):
    ...
    def train_interval(self, epochs, batch_size, train_generator,
                       val_generator, verbose=0):
        self.set_interval_trainable(split=-8)
        self.interval_unet.summary()
        #imi, imo = next(train_generator)
        #imp = self.single_unet.predict(imi[0])
        #lo, hi = self.interval_unet.predict(imi)
        #print(imi[0].min(), imi[0].max(), imi[1].min(), imi[1].max())
        #print((imi[1]-imi[0]).min(), (imi[1]-imi[0]).max())
        #print(imo[0].min(), imo[0].max(), imo[1].min(), imo[1].max())
        #print(imp.min(), imp.max())
        #print(lo.min(), lo.max())
        #print(hi.min(), hi.max())

        #for l in self.interval_unet.layers:
        #    if isinstance(l, Conv2D):
        #        tmp = K.function(self.interval_unet.inputs, l.get_output_at(1))
        #        out = tmp(imi)
        #        print(out[0].min(), out[0].max(), out[1].min(), out[1].max())
        #        print((out[1]-out[0]).min(), (out[1]-out[0]).max())
        #        print('---')
        for epoch in range(1, epochs+1):
            start = time.time()
