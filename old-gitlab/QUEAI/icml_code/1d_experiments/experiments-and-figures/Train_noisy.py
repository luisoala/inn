...
from Util import load_batch_pow_8_noisy
...
for i in range(num_training_runs):
	print('############################')
	print('Beginning noisy run [{}/{}]'.format(i+1, num_training_runs))
	print('############################')
	model = TestConvNet()

	model = do_base_training_run(model, num_epochs, learning_rate, batch_size, data_loading_function)
	torch.save(model.state_dict(), dir_path+'/ModelSaves/BaseModelNoise_run{}.pt'.format(i))

	model = do_interval_training_run(model, num_interval_epochs, interval_learning_rate, batch_size, beta, data_loading_function)
	torch.save(model.state_dict(), dir_path+'/ModelSaves/IntervalModelNoise_run{}.pt'.format(i))

	model = TestConvNet()
	model.load_state_dict(torch.load(dir_path+'/ModelSaves/BaseModelNoise_run{}.pt'.format(i)))
	model = do_probout_training_run(model, num_probout_epochs, probout_learning_rate, batch_size, data_loading_function)
	torch.save(model.state_dict(), dir_path+'/ModelSaves/ProboutModelNoise_run{}.pt'.format(i))