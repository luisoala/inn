...
class Dense(Layer):
    ...
    def build(self, input_shape):
        ...
        self.min_kernel = self.add_weight(shape=(input_dim, self.units),
                                      initializer='zeros',
                                      name='min_kernel',
        ...
        self.max_kernel = self.add_weight(shape=(input_dim, self.units),
                                      initializer='zeros',
                                      name='max_kernel',
                                      regularizer=self.minmax_kernel_regularizer,
                                      constraint=constraints.get('nonneg'),
        ...
        if self.use_bias:
            self.min_bias = self.add_weight(shape=(self.units,),
                                        initializer='zeros',
                                        name='min_bias',
            ...
            self.max_bias = self.add_weight(shape=(self.units,),
                                        initializer='zeros',
                                        name='max_bias',
                                        regularizer=self.minmax_bias_regularizer,
                                        constraint=constraints.get('nonneg'),
    ...
    def call(self, inputs):
        def _both_call(inputs):         ### TODO: refactor both call to use single and interval calls
            x, lo, hi = inputs
            x = K.maximum(x, 0.0)
            lo = K.maximum(lo, 0.0)     ### TODO NOT QUITE RIGHT FOR FIRST LAZER
            hi = K.maximum(hi, 0.0)
            lo_out = K.dot(lo, K.maximum(self.kernel-self.min_kernel, 0.0)) + K.dot(hi, K.minimum(self.kernel-self.min_kernel, 0.0))
            hi_out = K.dot(lo, K.minimum(self.kernel+self.max_kernel, 0.0)) + K.dot(hi, K.maximum(self.kernel+self.max_kernel, 0.0))
            output = K.dot(x, self.kernel)
            if self.use_bias:
                lo_out = K.bias_add(lo_out, self.bias-self.min_bias, data_format='channels_last')
                hi_out = K.bias_add(hi_out, self.bias+self.max_bias, data_format='channels_last')
                output = K.bias_add(output, self.bias, data_format='channels_last')
            if self.activation is not None:
                #assert activations.serialize(self.activation) == 'relu'
