...
class ContextEncoder():
    ...
    def thresh_relu(self, x):
        return K.relu(x, alpha = 0.0, max_value = None, threshold = -10. )
    
    def thresh_exp(self, x):
        return tf.cond(x < -10., lambda: tf.clip_by_value(x, -10., 1000.), lambda: tf.math.exp(x))
    ...
    def build_generator(self):
        ...
        #out_b = Activation(self.thresh_exp)(out_b)
    ...
    def sample_images(self, epoch, imgs, test_or_train):
        if test_or_train == 'train':
            ...
            matrices = {'imgs':imgs, 'masked_imgs':masked_imgs, 'gen_missing':gen_missing, 'missing_parts':missing_parts, 'out_b':out_b}
            ...
            for i in range(c):
                ...
                axs[2,i].axis('off')
            fig.savefig("images-train/%d.png" % epoch)
            plt.close()
