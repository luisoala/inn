...
class Unet_Prob():
	...
	def exponential_dist_loss(self, y_true, y_pred):
		...
		prevar = K.expand_dims(y_pred[:,:,:,1], axis=-1)
		term1 = K.sum(prevar)
		#term1 = K.sum(var)
		term2 = K.sqrt(K.sum(K.square(y_true - mean) / (K.exp(prevar)+self.eps)))
	...
	def build_unet(self):
		...
		conv10_mean = Conv2D(1, 1, activation = 'sigmoid')(resid) #TODO: what output activation they use?
		conv10_var = Conv2D(1, 1, activation = 'sigmoid')(resid)
		
		out = Concatenate(axis=-1)([conv10_mean, conv10_var])
