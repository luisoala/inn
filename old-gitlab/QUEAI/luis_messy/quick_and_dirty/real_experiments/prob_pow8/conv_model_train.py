...
MODEL_TYPE = sys.argv[2] #'prob', 'drop', 'det', 'error_pred', error_pred_propper, error_pred_propper_newinits
...
if MODEL_TYPE == 'error_pred':
	...
	for i in range(0, EPOCHS, SAMPLING_INTERVAL):
		convnet_pred.train(SAMPLING_INTERVAL, BATCH_SIZE, SAMPLING_INTERVAL, X_train, Y_train, X_val, Y_val, X_test, Y_test, False, False)
		...
		convnet_errorpred.train(101,BATCH_SIZE, 100, disc_input_train, Error_train, disc_input_val, Error_val, disc_input_test, Error_test, True, True, i) #train error prediction network
		

elif MODEL_TYPE == 'error_pred_propper_newinits':
	X_train, Y_train, X_val, Y_val, X_test, Y_test =  get_data(DATA_PATH) #load data for original prediction task
	convnet_pred = ConvNet_Det() #inititalize a prediction network instance
