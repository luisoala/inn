''' Moment propagating versions of several layers from
    tensorflow/python/keras/layers/convlutional.py
...
class Conv(Layer):
    ...
    Arguments:
        ...
        kernel_constraint: Optional projection function to be applied to the
            kernel after being updated by an `Optimizer` (e.g. used to
            implement norm constraints or value constraints for layer weights).
            The function must take as input the unprojected variable and must
            return the projected variable (which must have the same shape).
    ...
    def build(self, input_shape):
        ...
        if self.data_format == 'channels_first':
            ...
            channel_axis = -1
        if (input_shape[0][channel_axis].value is None
                or input_shape[1][channel_axis].value is None):
            raise ValueError('The channel dimension of the inputs '
                             'should be defined. Found `None`.')
        input_dim = int(input_shape[0][channel_axis])
        ...
        if self.use_bias:
            ...
            self.bias = None
        self.input_spec = [
            InputSpec(ndim=self.rank + 2, axes={channel_axis: input_dim}),
            InputSpec(ndim=self.rank + 2, axes={channel_axis: input_dim}),
        ]
    ...
    def call(self, inputs):
        ...
        outputs[1] = self._convolution_op(variances, K.square(self.kernel))
        ...
        if self.use_bias:
            if self.data_format == 'channels_first':
                if self.rank == 1:
                    ...
                    outputs[0] += bias
                if self.rank == 2:
                    outputs[0] = nn.bias_add(
                        outputs[0],
                        self.bias,
                        data_format='NCHW',
                    )
                if self.rank == 3:
                    # As of Mar 2017, direct addition is significantly slower
                    # than bias_add when computing gradients. To use bias_add,
                    # we collapse Z and Y into a single dimension to obtain a
                    # 4D input tensor.
                    outputs_shape = outputs[0].shape.as_list()
                    ...
                    outputs[0] = array_ops.reshape(outputs_4d, outputs_shape)
            else:
                outputs[0] = nn.bias_add(outputs[0],
                                         self.bias, data_format='NHWC')
