...
class ConvNet_Interval():
    ...
    def train(self, epochs, batch_size, sample_interval,
        ...
        for sample_iteration in range(epochs//sample_interval):
            self.single_training_run(sample_interval, batch_size, X_train, Y_train, X_val, Y_val)
            if save_params:
                self.single_convnet.save(
                    'convnet_interval_single_epoch' + str((sample_iteration+1)*sample_interval)+'.h5'
                )
            
            self.interval_training_run(50, batch_size, X_train, Y_train, X_val, Y_val, beta=-1)
            if save_params:
                self.interval_convnet.save(
                    'convnet_interval_interval_epoch' + str((sample_iteration+1)*sample_interval)+'.h5'
                )
            
            
            if save_samples:
                self.sample_images((sample_iteration+1)*sample_interval, X_test, Y_test, 'test')
